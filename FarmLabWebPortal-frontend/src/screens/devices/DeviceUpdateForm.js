...
export default function DeviceUpdateForm() {
  ...
  const [deviceSectorId, setDeviceSectorId] = useState("");
      ...
      getData({
        ...
        callback: (data) => {
          if (data.farm && data.farm === parseInt(currentFarmId)) {
            setDeviceFarmId(data.farm);
          }
          else {
            navigate("/farms");  // TODO: Must be changed to navigate("/farms/${currentFarmId}") after implementation
            toast.error("Thiết bị này không thuộc Farm Lab của bạn");
          }
          setDeviceSectorId(data.sector);
      ...
      >
        <div className="bg-white p-6 rounded-lg shadow-lg border w-full max-w-3xl mx-auto">
          ...
          <select
            value={deviceFarmId}
            onChange={(e) => {
              setDeviceFarmId(e.target.value);
              updateSectorList(e.target.value)
            }}
            required
          ...
          <select
            value={deviceSectorId || 0}
            onChange={(e) => setDeviceSectorId(e.target.value)}
            className="w-full p-2 border rounded-md mb-2"
            name="sector"
          >
            {
              farmLabs.filter(farmLab => farmLab.id === parseInt(deviceFarmId)).map((farmLab) => (
                farmLab.sectors.map((sector) => (
